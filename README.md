...
- 스위치백 트랙 마블 레이스 + **선두를 따라가는 카메라**
- **미니맵**(전체 트랙·카메라 뷰포트) + **실시간 순위판**
- 회전 스피너 장애물, 하단 깔때기 골인 지점
- 유머: 실시간 중계 드립, 도착 순간 멘트, 시상식 밈, 효과음, 골인 꽃가루
- 완전 순위 결과 — 1등부터 꼴찌까지 등수 + 재미있는 코멘트
- 클로드 감성 디자인(따뜻한 크림·코랄 톤)
- 의존성 하나(로컬 번들된 `matter.min.js`)뿐인 정적 사이트 — 빌드 불필요
- 물리 검증: 인원수 2~24명 각각 헤드리스 시뮬레이션으로 100% 완주·잼 없음·타이밍 21~28초·공정성(Spearman 0.04) 확인
